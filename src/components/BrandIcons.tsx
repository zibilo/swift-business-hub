...
export type BrandIconName =
  | 'dashboard' | 'business' | 'import' | 'history' | 'support'
  | 'profile' | 'add' | 'edit' | 'delete' | 'search' | 'download'
  | 'close' | 'menu' | 'logout' | 'alert' | 'success' | 'info'
  | 'security' | 'email' | 'lock' | 'message';
...
export const BrandIcon = ({ name, className = "h-5 w-5", size, color = "currentColor" }: BrandIconProps) => {
  const getIconPath = () => {
    switch (name) {
      case 'dashboard':
        return <path d="M3 3h8v10H3V3zm10 0h8v6h-8V3zm0 8h8v10h-8V11zM3 15h8v6H3v-6z" />;
      case 'business':
      ...
      case 'history':
        return <path d="M13 3c-4.97 0-9 4.03-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42C8.27 19.99 10.51 21 13 21c4.97 0 9-4.03 9-9s-4.03-9-9-9zm-1 5v5l4.28 2.53.85-1.27-3.63-2.16V8H12z" />;
      case 'support':
        return <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17h-2v-2h2v2zm2.07-7.75l-.9.92C13.45 12.9 13 13.5 13 15h-2v-.5c0-1.1.45-2.1 1.17-2.83l1.24-1.26c.37-.36.59-.86.59-1.41 0-1.1-.9-2-2-2s-2 .9-2 2H8c0-2.21 1.79-4 4-4s4 1.79 4 4c0 .88-.36 1.68-.93 2.25z" />;
      case 'profile':
      ...
      case 'logout':
        return <path d="M13 3h-2v10h2V3zm4.83 2.17l-1.42 1.42C17.99 7.86 19 9.81 19 12c0 3.87-3.13 7-7 7s-7-3.13-7-7c0-2.19 1.01-4.14 2.58-5.42L6.17 5.17C4.23 6.82 3 9.26 3 12c0 4.97 4.03 9 9 9s9-4.03 9-9c0-2.74-1.23-5.18-3.17-6.83z" />;
      case 'close':
      ...
      case 'message':
        return <path d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-2 12H6v-2h12v2zm0-3H6V9h12v2zm0-3H6V6h12v2z" />;
      default:
        return null;
    }
  };
